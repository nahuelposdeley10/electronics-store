...
export default function OrderStatus({ status, orderId, onNavigate }) {
  ...
  useEffect(() => {
    ...
    apiConfirmOrder(orderId)
      ...
      .catch(() => {
        setCheckError('No pudimos confirmar el estado del pedido.')
      })
    return () => {
      unsubscribe()
    }
  ...
  if (PENDING_STATUSES.has(current)) {
    return (
      <main className="cart cart-empty">
        ...
        {checkError && (
          <p className="order-check-error" role="alert">
            {checkError} Si ya realizaste el pago, vas a verlo reflejado acá.
          </p>
        )}
        <button type="button" className="primary-btn" onClick={() => onNavigate('home')}>
          Seguir comprando
